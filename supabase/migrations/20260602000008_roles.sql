...
--
-- Role values (stored in auth.users.raw_app_meta_data ->> 'role'):
--   admin   — full access
--   finance — accounting status + financial events + settings read
--   ops     — factories / supervisors / jobs / operational status
--   viewer  — read only (default)
--
-- All write RPCs are re-created here with assert_role() guards.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- current_user_role()
-- Reads role from JWT app_metadata (appears at top level in Supabase JWTs).
-- Falls back to 'viewer' when not set.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- assert_role(allowed_roles)
-- Raises permission_denied if current_user_role() is not in allowed_roles.
-- ---------------------------------------------------------------------------
...
BEGIN
  IF NOT (v_role = ANY(p_allowed_roles)) THEN
    RAISE EXCEPTION 'PERMISSION_DENIED: role % is not allowed for this operation. Required: %',
      v_role, array_to_string(p_allowed_roles, ', ')
      USING ERRCODE = 'P0001';
  END IF;
END;
$func$;
...
-- =============================================================================
-- Re-create write RPCs with role guards
-- Pattern: PERFORM assert_role(...) right after PERFORM set_audit_context(...)
-- =============================================================================

-- ---------------------------------------------------------------------------
-- rpc_create_factory  →  admin, ops
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  PERFORM assert_role(ARRAY['admin','ops']);
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE EXCEPTION 'OPTIMISTIC_LOCK_CONFLICT_OR_NOT_FOUND: factory %, expected version %',
      p_factory_id, p_expected_version USING ERRCODE = 'P0001';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rpc_archive_factory  →  admin only
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_active_jobs > 0 THEN
    RAISE EXCEPTION 'PRECONDITION_FAILED: factory % has % unresolved job(s)',
      p_factory_id, v_active_jobs USING ERRCODE = 'P0001';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rpc_create_supervisor  →  admin, ops
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- rpc_update_supervisor  →  admin, ops
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE EXCEPTION 'OPTIMISTIC_LOCK_CONFLICT_OR_NOT_FOUND: supervisor %, expected version %',
      p_supervisor_id, p_expected_version USING ERRCODE = 'P0001';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rpc_archive_supervisor  →  admin only
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_active_jobs > 0 THEN
    RAISE EXCEPTION 'PRECONDITION_FAILED: supervisor % has % unresolved job(s)',
      p_supervisor_id, v_active_jobs USING ERRCODE = 'P0001';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'NOT_FOUND_OR_INACTIVE: factory % in org %',
      p_factory_id, p_organization_id USING ERRCODE = 'P0001';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'NOT_FOUND_OR_INACTIVE: supervisor % in org %',
      p_supervisor_id, p_organization_id USING ERRCODE = 'P0001';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rpc_advance_operational_status  →  admin, ops
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE EXCEPTION 'OPTIMISTIC_LOCK_CONFLICT_OR_NOT_FOUND: job %, expected version %',
      p_job_id, p_expected_version USING ERRCODE = 'P0001';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rpc_advance_accounting_status  →  admin, finance
-- ---------------------------------------------------------------------------
...
BEGIN
  PERFORM set_audit_context(p_actor_id, p_correlation_id);
  PERFORM assert_role(ARRAY['admin','finance']);
...
-- ---------------------------------------------------------------------------
-- rpc_append_financial_event  →  admin, finance
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- rpc_upsert_flex_field_definition  →  admin only
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO flex_field_definitions (
    ...
    field_type, display_order, is_required, enum_options
  ) VALUES (
    p_organization_id, p_entity_type, p_field_key, p_label,
    p_field_type, p_display_order, p_is_required, p_enum_options
  )
  ...
  INSERT INTO idempotency_log (key, organization_id, operation, result_summary)
  VALUES (p_idempotency_key, p_organization_id, 'upsert_flex_field', v_row.id::text)
...
-- ---------------------------------------------------------------------------
-- rpc_upsert_system_setting  →  admin only
-- ---------------------------------------------------------------------------
...
BEGIN
  PERFORM set_audit_context(p_actor_id, 'settings-upsert');
  PERFORM assert_role(ARRAY['admin']);

  INSERT INTO system_settings (organization_id, key, value, description)
  VALUES (p_organization_id, p_key, p_value, p_description)
